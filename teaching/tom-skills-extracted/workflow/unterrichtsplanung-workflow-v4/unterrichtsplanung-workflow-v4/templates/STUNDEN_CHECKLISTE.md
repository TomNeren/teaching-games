...
- [ ] Vorlage korrekt geladen: `Vorlage_Fach.docx`
...
**Bild-Prüfungen:**
- [ ] ComfyUI gestartet und Server erreichbar
- [ ] Primär: `comfyui:generate_image` aufgerufen
- [ ] Bei Fehler: Fallback verwendet (nanobanana / HF)
- [ ] Format korrekt: 1280x720 (16:9)
...
**Validierungsprüfungen:**
- [ ] Vorlage korrekt geladen: `Vorlage.pptx`
- [ ] Folienanzahl korrekt: {{ANZAHL_IST}} / {{ANZAHL_SOLL}}
- [ ] Timer-Videos eingefügt: {{TIMER_ANZAHL}}
...
def validate_presentation(pptx_path: str, expected_slides: int, expected_timers: int) -> bool:
    ...
    Prüft: Folienanzahl, Timer, Umlaute, Fonts
    """
    ...
    # Check 2: Videos/Timer zählen
    video_count = 0
    for slide in prs.slides:
        for shape in slide.shapes:
            if hasattr(shape, "media_type") or shape.shape_type == 3:  # MEDIA
                video_count += 1
    
    if video_count < expected_timers:
        errors.append(f"❌ TIMER/VIDEOS: {video_count} (erwartet: min. {expected_timers})")
...
| **4.0** | **24.01.2025** | **ComfyUI-Status, v2-Validierungsfunktionen, Font-Check** |
