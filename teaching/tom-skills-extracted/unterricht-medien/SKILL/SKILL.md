...
# Unterrichts-Medien Skill v5.0

Erstellt visuelle Unterrichtsmaterialien über ComfyUI (lokal bevorzugt) oder Fallback-Systeme.

**NEU in v5.0:** MCP-Tool-Namen vereinheitlicht (mcp__...), Synchronisiert mit unterrichtsplanung-workflow v5.0
...
**Tool-Name:** `mcp__comfyui__generate_image`
...
```javascript
mcp__fa86031e-8beb-4aac-b517-e3b796ddf8ec__gr1_flux_1_kontext_dev_infer({
...
```javascript
mcp__fa86031e-8beb-4aac-b517-e3b796ddf8ec__gr4_z_image_turbo_generate({
  prompt: "[Prompt]",
...
```
┌─────────────────────────────────────────────────────────────────┐
│ 1. Versuche: mcp__comfyui__generate_image                             │
├─────────────────────────────────────────────────────────────────┤
...
│         ↓                                                       │
│ 4. Versuche erneut: mcp__comfyui__generate_image                      │
│         ↓                                                       │
│ Erfolg? → Bild generiert ✅                                     │
...
```javascript
mcp__Control_your_Mac__osascript({ script: 'do shell script "curl -s -m 5 http://127.0.0.1:8188/system_stats"' })
```

**ComfyUI starten:**
```javascript
mcp__Control_your_Mac__osascript({ script: 'do shell script "cd ~/ComfyUI && source venv/bin/activate && python main.py --use-pytorch-cross-attention --force-fp16 &> /tmp/comfyui.log &\necho started"' })
```

**Warten:**
```javascript
mcp__Control_your_Mac__osascript({ script: 'delay 8' })
```

**Bild generieren (PPTX 16:9):**
```javascript
mcp__comfyui__generate_image({ prompt: "[...]", width: 1024, height: 576, seed: -1 })
```

**Bild generieren (Quadrat):**
```javascript
mcp__comfyui__generate_image({ prompt: "[...]", width: 1024, height: 1024, seed: -1 })
```
...
|---------|-------|------------|
| **5.0.0** | **29.01.2026** | **MCP-Tool-Namen vereinheitlicht (mcp__...), Synchronisiert mit unterrichtsplanung-workflow v5.0** |
| 3.1.0 | 27.01.2026 | Robusterer Workflow mit Retry-Logik; PPTX-Design-System-Integration |
| 3.0.0 | 27.01.2026 | Auto-Start ComfyUI via osascript |
| 2.0.0 | 20.01.2026 | FLUX-optimierte Prompts |
| 1.0.0 | 15.01.2026 | Initial Release |

---

*Version: 5.0.0 - MCP-Tool-Namen vereinheitlicht*
